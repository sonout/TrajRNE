...
class SpatialFlowConvolution(Model):
    def __init__(
        ...
        self.model = SFCSubConv(data.x.shape[1], emb_dim, norm=norm)
        ...
        if adj is None and traj_data is not None:
            ...
            adj = SpatialFlowConvolution.generate_node_traj_adj(
                traj_data, network, k=k, bidirectional=bidirectional, add_self_loops=add_self_loops
            )
            np.savetxt(
                "./data/traj_adj_k_" + str(k) + "_" + str(bidirectional) + ".gz", X=adj
            )
        # else:
